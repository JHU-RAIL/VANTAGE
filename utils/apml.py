...
class WeightedAdaptiveProbabilisticMatchingLoss(nn.Module):
    """
    Adaptive Probabilistic Matching Loss (APML) for point cloud comparison.
    Approximates optimal transport using a temperature-scaled softmin that ensures
    a minimum assignment probability, followed by Sinkhorn normalization.
    """
    ...
    @torch.no_grad()
    def _weighted_sinkhorn(
        self,
        P: Tensor,
        mass_A: Optional[Tensor] = None,
        mass_B: Optional[Tensor] = None,
        iterations: int = 20
    ) -> Tensor:
        """Performs Sinkhorn normalization on a soft assignment matrix."""

        if mass_A is None:
            mass_A = torch.ones(P.size(0), P.size(1), device=P.device)

        if mass_B is None:
            mass_B = torch.ones(P.size(0), P.size(2), device=P.device)

        mass_A = mass_A.view(P.size(0), P.size(1), 1)
        mass_B = mass_B.view(P.size(0), 1, P.size(2))

        for _ in range(iterations):
            P = P * (mass_A / (P.sum(dim=2, keepdim=True) + self.eps))
            P = P * (mass_B / (P.sum(dim=1, keepdim=True) + self.eps))
        return P

    def forward(self, predicted: Tensor, ground_truth: Tensor, pred_pc_mass: Optional[Tensor] = None) -> Tensor:
        """
        Compute the Adaptive Probabilistic Matching Loss between predicted and ground truth point clouds.

        Args:
            predicted: Tensor of shape [B, N, d]
            ground_truth: Tensor of shape [B, M, d]
            pred_mass: Tensor of shape [B, N]
        ...
        if pred_pc_mass is not None:
            pred_pc_mass = torch.clamp(pred_pc_mass, min=self.eps)
            pred_pc_mass = predicted.size(1) * pred_pc_mass / torch.sum(pred_pc_mass, dim=1, keepdim=True)
            # print(pred_pc_mass)
            # print(pred_pc_mass.max())
            # print(pred_pc_mass.min())
            # print(pred_pc_mass.mean())
            # print(pred_pc_mass.std())
            # assert False
        ...
        with torch.no_grad():
            ...
            P2 = self._softmin_with_min_probability(cost, dim=1)
            P = 0.5 * (P1 + P2)
            P = self._weighted_sinkhorn(P, mass_A=pred_pc_mass)
